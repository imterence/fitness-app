"use client"

import { useState } from "react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Upload, Download, FileText, AlertCircle, CheckCircle, Loader2 } from "lucide-react"
import { parse } from "csv-parse/sync"

interface ExerciseCSVRow {
  name: string
  category: string
  description: string
  difficulty: string
  instructions: string
  videoUrl: string
}

interface ParsedExercise {
  name: string
  description: string
  category: string
  difficulty: string
  instructions: string
  videoUrl?: string
}

interface ExerciseCSVImportProps {
  onImport: (exercises: ParsedExercise[]) => void
  onCancel: () => void
}

export default function ExerciseCSVImport({ onImport, onCancel }: ExerciseCSVImportProps) {
  const [file, setFile] = useState<File | null>(null)
  const [isProcessing, setIsProcessing] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [preview, setPreview] = useState<ParsedExercise[]>([])

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFile = e.target.files?.[0]
    if (selectedFile && selectedFile.type === "text/csv") {
      setFile(selectedFile)
      setError(null)
      setPreview([])
    } else {
      setError("Please select a valid CSV file")
      setFile(null)
    }
  }

  const downloadTemplate = () => {
    const link = document.createElement("a")
    link.href = "/exercise-template.csv"
    link.download = "exercise-template.csv"
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  }

  const processCSV = async () => {
    if (!file) return

    setIsProcessing(true)
    setError(null)

    try {
      const text = await file.text()
      const rows = parse(text, {
        columns: true,
        skip_empty_lines: true,
        trim: true
      }) as ExerciseCSVRow[]

      const exercises = parseExerciseRows(rows)
      setPreview(exercises)
    } catch (err) {
      setError("Error processing CSV file. Please check the format.")
      console.error("CSV processing error:", err)
    } finally {
      setIsProcessing(false)
    }
  }

  const parseExerciseRows = (rows: ExerciseCSVRow[]): ParsedExercise[] => {
    return rows.map((row) => ({
      name: row.name,
      description: row.description,
      category: row.category,
      difficulty: row.difficulty,
      instructions: row.instructions || "",
      videoUrl: row.videoUrl || undefined
    }))
  }

  const handleImport = () => {
    if (preview.length > 0) {
      onImport(preview)
    }
  }

  return (
    <Card className="w-full max-w-4xl mx-auto bg-white border border-gray-200 shadow-xl">
      <CardHeader className="bg-gray-50 border-b border-gray-200">
        <CardTitle className="flex items-center gap-2 text-gray-900">
          <FileText className="h-5 w-5 text-blue-600" />
          Import Exercises from CSV
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6 p-6">
        {/* Template Download */}
        <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
          <div>
            <h3 className="font-medium text-blue-900">Download Template</h3>
            <p className="text-sm text-blue-700">
              Use our CSV template to ensure proper formatting for exercises. Only the exercise name is required.
            </p>
          </div>
          <Button onClick={downloadTemplate} variant="outline" size="sm">
            <Download className="h-4 w-4 mr-2" />
            Download Template
          </Button>
        </div>

        {/* File Upload */}
        <div className="space-y-4">
          <Label htmlFor="csv-file">Select CSV File</Label>
          <Input
            id="csv-file"
            type="file"
            accept=".csv"
            onChange={handleFileChange}
            className="cursor-pointer"
          />
          {file && (
            <div className="flex items-center gap-2 text-sm text-green-600">
              <CheckCircle className="h-4 w-4" />
              {file.name} selected
            </div>
          )}
        </div>

        {/* Process Button */}
        {file && (
          <Button 
            onClick={processCSV} 
            disabled={isProcessing}
            className="w-full"
          >
            {isProcessing ? (
              <>
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                Processing CSV...
              </>
            ) : (
              <>
                <Upload className="h-4 w-4 mr-2" />
                Process CSV
              </>
            )}
          </Button>
        )}

        {/* Error Display */}
        {error && (
          <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700">
            <AlertCircle className="h-4 w-4" />
            {error}
          </div>
        )}

        {/* Preview */}
        {preview.length > 0 && (
          <div className="space-y-4">
            <h3 className="font-medium text-gray-900">Preview ({preview.length} exercises)</h3>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {preview.map((exercise, index) => (
                <div key={index} className="p-3 border border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100">
                  <div className="font-medium text-gray-900">
                    {exercise.name}
                  </div>
                  <div className="text-sm text-gray-600 mt-1 line-clamp-2">
                    {exercise.description}
                  </div>

                </div>
              ))}
            </div>
          </div>
        )}

        {/* Action Buttons */}
        <div className="flex gap-3 pt-4">
          <Button onClick={onCancel} variant="outline" className="flex-1">
            Cancel
          </Button>
          {preview.length > 0 && (
            <Button onClick={handleImport} className="flex-1">
              Import {preview.length} Exercise{preview.length !== 1 ? 's' : ''}
            </Button>
          )}
        </div>
      </CardContent>
    </Card>
  )
}
